...
class Hold:
    def __init__(self, Admin_id):
        ...
        def Find():
            ...
            q = "select ID,name,age,gender,post,contact,address,isin,hold from userse where Name like'%"+str(name)+"%'  and post='"+str(Department)+"' and isin=1 and hold=0"
            print(q)
            cursor.execute(q)
            ...
            if (len(result) == 0 or result == None):
                ...
                top.destroy()
                #root.destroy()
                HList.ListKAKA(result, Admin_id)
