...
class EnterData():
    ...
    def __init__(self,ID,Admin_id):
        ...
        def on_closing():
            ...
            if messagebox.askokcancel("Quit", "You will be redirected to the main page... \n Do you wish to continue ?"):
        ...
        def Provide():
            ...
            if self.IDdb == '' or self.NAMEdb == '' or self.AGEdb == '' or self.GENDERdb == '' or self.POSTdb == '' or self.CONTACTdb == '' or len(self.ADDRESSdb) < 2:
                
                
                messagebox.showerror("Information Error", "Please Fillout all the information properly")
                return 0
            
            if str(self.AGEdb).isdigit():
                if int(self.AGEdb) > 50 :
                    messagebox.showerror("Age More than usual", "Please Select the Age in between 18 to 50")
                    return 0
                elif int(self.AGEdb) < 18:
                    messagebox.showerror("Age Less than usual", "Please Select the Age in between 18 to 50")
                    return 0
            else:
                
                messagebox.showerror("The Data error", "Please Keep the number as the age... ")
                return 0
            
            if self.POSTdb == "--- Select Post ---" :
                messagebox.showerror("The Post selection error ", "Please Select some post... ")
                return 0
            ...
            window.destroy()
            root.destroy()
            CaptureScreen.Capture((self.IDdb,self.NAMEdb,self.AGEdb,self.GENDERdb,self.POSTdb,self.CONTACTdb,self.ADDRESSdb,Admin_id))
            # Passing ID,Name,Age,G,Post,Contact,Address,Admin Id ( Total 8 )
        ...
        TCombobox1 = ttk.Combobox(window,value = posts,textvariable = postValue, state = 'readonly')
